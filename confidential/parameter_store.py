...
class ParameterStore:
    ...
    def __getitem__(self, key):
        """
        Allows us to do <ParameterStore>["foo"] instead of <ParameterStore>.secrets.get("foo")
        """
    ...
    def decrypt_secret_from_aws(self, secret_name) -> str:
        ...
        try:
            ...
            if e.response["Error"]["Code"] == "DecryptionFailureException":
                raise Exception("can't decrypt the secret text. Check your permissions.") from e
                ...
                raise Exception(f"We can't find the parameter named {secret_name}.") from e
        else:
            return get_secret_value_response["Parameter"]["Value"] if "Parameter" in get_secret_value_response else None
    ...
    def parse_secrets_file(self, path_to_file) -> dict:
        ...
        for key, value in config.items():
            if isinstance(value, str) and value.startswith("parameter:"):
                decrypted_string = self.decrypt_secret_from_aws(value[10:])
...
def decrypt_secret(secrets_file, default_secrets_file, aws_region, output_json):
    ...
    secrets_manager = ParameterStore(secrets_file, default_secrets_file, aws_region)
